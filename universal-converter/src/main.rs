...
fn main() {
    ...
    let conversion_type = match args.conversion_type {
        Some(conversion_type) => Some(conversion_type),
        None => {
            // Attempt to infer the conversion type
            match ConversionType::infer_conversion_type(&args.from_unit, &args.to_unit) {
    ...
    match conversion_type {
        ...
        None => {
            // This case should not be reached because all possibilities are covered above
